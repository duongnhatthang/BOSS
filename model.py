...
def sherman_morrison(X, V, w=1):
    result = V - (w * np.einsum("ij,j,k,kl -> il", V, X, X, V)) / (
        1.0 + w * np.einsum("i,ij,j ->", X, V, X)
    )
    return result


# For nice printing
def round(X, precision=0.03):
...
class UCB:
    def __init__(self, d, alpha, unit_ball_action=True, lam=1):
        ...
        if unit_ball_action:
            self.contexts = generate_contexts(
                n_gen_context=20, d=d
            )  # TODO: random seed here if needed later
        self.reset()
...
class PHE:
    ...
    def update(self, reward):
        ...
        self.Binv = sherman_morrison(self.X_a, self.Binv)
        self.yx = np.sum(
            np.multiply(np.array(self.context_list), pseudo_reward), axis=0
        )
        self.theta_hat = self.Binv @ self.yx
    ...
    def select_ctx(self, contexts):
        if self.step <= self.tau_1:
            if self.EXR_contexts is None:
                ...
                u = 1
                self.X_a = (
                    u * self.X_a / np.linalg.norm(self.X_a)
...
class PMA(BOSS_protocol):
    def __init__(self, input_dict, true_B, lam=1):
        super().__init__(
            input_dict, input_dict["PMA_tau2_const"], input_dict["PMA_stop_exr"]
        )
        ...
        if C_info > self.C_miss:
            ...
            self.C_miss *= c
        assert (
            self.C_hit <= self.C_miss
        ), f"self.C_hit ({round(self.C_hit)}) > self.C_miss ({self.C_miss})"
        assert (
            self.C_hit <= C_info
        ), f"self.C_hit ({round(self.C_hit)}) > C_info ({C_info})"
        assert (
            C_info <= self.C_miss
        ), f"C_info ({round(C_info)}) > self.C_miss ({self.C_miss})"
        ...
        self.p = self.input_dict["PMA_exr_const"] * np.sqrt(
            T
            * d
            * m
            / (n_task * (d ** (4 / 3) * T ** (1 / 3) + d ** (2 / 3) * T ** (2 / 3)))
        )
        self.reset()
        logger.info(
            f"PMA's exp prob = {round(self.p)}, tau_1 = {round(self.tau_1)}, tau_2 = {round(self.tau_2)}"
        )
    ...
    def update_B_hat(self):
        ...
        if self.is_first_round:
            self.q = [
                1 / PMA_n_expert
            ] * PMA_n_expert  # distribution to sample the expert
        else:
            if self.is_EXR:  # Update self.q here
                ...
                if all_loss_equal:
                    if l_i == 0:
                        logger.info(
                            f"Warning: check_alpha_cover fails (covers all experts) l={l_i}. Decrease PMA_alpha_const."
                        )
                    else:
                        logger.info(
                            f"Warning: check_alpha_cover fails (rejects all experts) l={l_i}. Increase PMA_alpha_const."
                        )
        ...
        if entropy_q == entropy([1 / PMA_n_expert] * PMA_n_expert):
            logger.info(
                "Warning: PMA's expert dist is still uniform. Reduce PMA_alpha_const or increase PMA_tau1_const"
            )
        if (
            self.input_dict["PMA_no_oracle"] == False
            and self.q[-1] < 0.9
            and self.input_dict["PMA_n_expert"] < 21
        ):
...
class SeqRepL(BOSS_protocol):
    def __init__(self, input_dict, lam=1):
        super().__init__(
            input_dict, input_dict["SeqRepL_tau2_const"], input_dict["SeqRepL_stop_exr"]
        )
        self.lam = lam
        self.others = []
        self.theta_hat_list = []
        ...
        self.reset()
        logger.info(
            f"SeqRepL's exp prob = {round(self.p)}, tau_1 = {round(self.tau_1)}, tau_2 = {round(self.tau_2)}"
        )
        ...
        if self.input_dict["SeqRepL_exr_list"] is not None:
            if self.task_idx in self.input_dict["SeqRepL_exr_list"]:
                ...
                self.base_model = PEGE(
                    d=self.input_dict["d"], tau_1=self.tau_1, lam=self.lam
                )
            else:
                self.is_EXR = False
                self.base_model = PEGE(
