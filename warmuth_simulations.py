...
def experiment_subset_selection_regret(
    ...
    for trial in range(n_trials):
        ...
        # First k experts: low loss → should be DISCARDED (we pay for them)
        # Remaining d experts: high loss → should be SELECTED (we don't pay)
        all_losses = np.zeros((T, n))
        for t in range(T):
            noise = rng.uniform(0, 1, size=n)
            # Low-loss experts (first k): should be discarded
            all_losses[t, :k] = 0.3 + 0.2 * noise[:k]
            # High-loss experts (remaining d): should be selected
            all_losses[t, k:] = 0.5 + 0.4 * noise[k:]
...
def experiment_comparison(
    ...
    for trial in range(n_trials):
        ...
        all_losses = np.zeros((T, n))
        for t in range(T):
            all_losses[t, :k] = rng.uniform(0.1, 0.5, size=k)  # Low loss → discard
            all_losses[t, k:] = rng.uniform(0.5, 0.9, size=d)  # High loss → select
...
def experiment_scaling_analysis():
    ...
    for T_idx, T in enumerate(T_values):
        ...
        for trial in range(n_trials):
            ...
            for t in range(T):
                ...
                all_losses[t, k:] = rng.uniform(0.6, 1.0, size=n-k)
    ...
    for d_idx, d in enumerate(d_values):
        ...
        for trial in range(n_trials):
            ...
            for t in range(T):
                all_losses[t, :k] = rng.uniform(0, 0.3, size=k)
                all_losses[t, k:] = rng.uniform(0.6, 1.0, size=d)
...
def run_all_experiments(quick_mode: bool = True):
    """Run all experiments.
    ...
    print("[1/5] experiment_capping_analysis...", flush=True)
    experiment_capping_analysis()
    print("Done!\n", flush=True)
    
    print("[2/5] experiment_marginal_probabilities...", flush=True)
    experiment_marginal_probabilities()
    print("Done!\n", flush=True)
    
    print("[3/5] experiment_subset_selection_regret...", flush=True)
    experiment_subset_selection_regret(n=10, k=5, T=T, n_trials=n_trials, seed=42)
    print("Done!\n", flush=True)
    
    print("[4/5] experiment_comparison...", flush=True)
    experiment_comparison(n=10, k=5, T=T, n_trials=n_trials, seed=42)
    print("Done!\n", flush=True)
    
    print("[5/5] experiment_scaling_analysis...", flush=True)
